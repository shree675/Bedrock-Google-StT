...
<div class="flex flex-row relative">
  ...
  <div @click="isOpen = !isOpen" class="cursor-pointer rounded-full ml-3 my-auto h-10 w-10 flex" :class="` +
  "`bg-${currentColor}`" +
  `>
    <svg xmlns="http://www.w3.org/2000/svg" :class="` +
  "`${iconColor}`" +
  `class="h-6 w-6 mx-auto my-auto" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <template x-for="color in colors">
          <div class="">
            <template x-for="variant in variants">
              <div @click="selectColor(color,variant)" class="cursor-pointer w-6 h-6 rounded-full mx-1 my-1" :class="` +
  "`bg-${color}-${variant}`" +
  `></div>
            </template>
          </div>
